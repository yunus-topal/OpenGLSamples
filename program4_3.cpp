...
void Program4_3::init(GLFWwindow* window) {
	renderingProgram = shader_utils::createShaderProgram("vertShader_4_3.glsl", "fragShader_4_3.glsl");
	cameraX = 0.0f; cameraY = 0.0f; cameraZ = 58.0f;
	cubeLocX = 0.0f; cubeLocY = -2.0f; cubeLocZ = 0.0f;
	pyrLocX = 2.0f; pyrLocY = 0.0f; pyrLocZ = 1.0f;
	setupVertices();
}
void Program4_3::display(GLFWwindow* window, double currentTime) {
	...
	glEnable(GL_DEPTH_TEST);
	glDepthFunc(GL_LEQUAL);
	glDrawArraysInstanced(GL_TRIANGLES, 0, 18, 5);
}

Program4_3::Program4_3() {
	std::cout << "Program 4_3 class constructor" << std::endl;
}

Program4_3::~Program4_3() {
	std::cout << "Program4_3 class destructor" << std::endl;
}

void Program4_3::execute() {
	if (!glfwInit()) { exit(EXIT_FAILURE); }
	glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 4);
	glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 3);
	GLFWwindow* window = glfwCreateWindow(600, 800, "Chapter4 - program3", NULL, NULL);
...
void Program4_3::setupVertices(void) {
...
glm::mat4 Program4_3::moveCube(double currentTime) {
